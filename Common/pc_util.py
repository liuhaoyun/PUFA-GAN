...
def normalize_point_cloud(input):
    ...
    input = input - centroid  # 去均值化
    furthest_distance = np.amax(
        np.sqrt(np.sum(input ** 2, axis=-1, keepdims=True)), axis=axis, keepdims=True)  # furthest_distance ：所有点中距离 质心点 centroid最远距离 用来归一化
    input = input / furthest_distance # 归一化
