...
from Common import pc_util,ops
from tf_ops.grouping.tf_grouping import query_ball_point, group_point, knn_point,knn_point_2
from tf_ops.sampling.tf_sampling import gather_point, farthest_point_sample
import numpy as np
import math

def cosine_similar_loss(xyz, feature, knn = 6, delta = 1.0e-5):
    N = xyz.get_shape().as_list()[1]
    _, _, idx = ops.get_KNN_feature(xyz, k=knn) # geometry space knn idx
    knn_feature = tf.gather_nd(feature, idx) # feature space knn [B,N,K,C]
    feature = tf.expand_dims(feature, axis = 2)
    feature_duplicate = tf.tile(feature, [1, 1, knn, 1])  # [B,N,K,C]
    # dot
    dot = feature_duplicate * knn_feature
    dot = tf.reduce_sum(dot, axis=-1) # [B,N,K]
    # norm
    knn_feature_norm = tf.sqrt(tf.reduce_sum(tf.square(knn_feature), axis=-1)) # [B,N,K]
    feature_duplicate_norm = tf.sqrt(tf.reduce_sum(tf.square(feature_duplicate),axis=-1)) # [B,K,K]
    norm = feature_duplicate_norm * knn_feature_norm  # [B,N,K]
    min_value = tf.ones_like(norm) * delta
    norm = tf.where(norm >= delta , x=norm, y=min_value) # 保证除数最小值不是0 [B,N,K]
    # cosine
    cosine = dot / norm
    cosine = tf.reduce_sum(cosine, axis=-1) # [B,N] 获取k个近邻点的 余弦相似度 的和
    cosine = tf.reduce_sum(cosine, axis=-1) # [B]
    cosine_loss = cosine / N # [B] # 求出整个点云的 相似度 loss
    cosine_loss = tf.reduce_mean(cosine_loss)  # 最终loss为 所有 Batch 平均值

    return cosine_loss

def cosine_similar_loss_var(xyz, feature, knn = 6, delta = 1.0e-5):
    N = xyz.get_shape().as_list()[1]
    _, _, idx = ops.get_KNN_feature(xyz, k=knn) # geometry space knn idx
    knn_feature = tf.gather_nd(feature, idx) # feature space knn [B,N,K,C]
    feature = tf.expand_dims(feature, axis = 2)
    ...
    norm = tf.where(norm >= delta , x=norm, y=min_value) # 保证除数最小值不是0 [B,N,K]
    # cosine
    cosine = dot / norm # [B,N,K]
    cosine_K = tf.reduce_mean(cosine, axis=-1,keep_dims=True) # [B,N,1] 获取k个近邻点的 余弦相似度平均值
    cosine_mean = tf.tile(cosine_K,[1,1,knn]) #[B,N,K] !!!!!
    cosine_var = tf.reduce_mean(tf.square(cosine - cosine_mean), axis = -1) # [B,N]  /k
    cosine_var = tf.reduce_mean(cosine_var, axis = -1) #[B] /N
    cosine_var_loss = tf.reduce_mean(cosine_var) # 最终loss为 所有 Batch 平均值 /B

    return cosine_var_loss
...
##################################################################################
#   Double Head Discriminator loss
##################################################################################

def DH_discriminator_loss(D, input_real, edge_input_real, input_fake, edge_input_fake):
    real = D(input_real, edge_input_real)
    fake = D(input_fake, edge_input_fake)
    real_loss = tf.reduce_mean(tf.square(real - 1.0))
    fake_loss = tf.reduce_mean(tf.square(fake))
...
def DH_generator_loss(D,input_fake, edge_input_fake):
    fake = D(input_fake,edge_input_fake)
